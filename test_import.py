...
def gen_task(q_list):
    ...
    for mysqlinfo in mysqlinfox.values():
        ...
        tablelist = []
        mysql_info = copy.copy(mysqlinfo)
        ...
        for row in partitioncur:
            ...
            if tablename == table_name:
                ...
                #task_count = task_count + 1
                #task_count_lock.release()
                #logging.info("task info %s", t)
                #q_list[p].put(t)
        partitiondb.close()
        mysql_instance = {}
        mysql_instance['info']=mysql_info
        mysql_instance['tables'] = tablelist
        logging.info("mysql instance infomation %s", mysql_instance)
        q_list[taskq_list_index].put(mysql_instance)
        task_count_lock.acquire()
        task_count = task_count + 1
        task_count_lock.release()
        taskq_list_index = taskq_list_index + 1 
