...
public class Mysql2HbaseCopy {
   ...
   static HbaseClientCopy client = null;
   static long count;
   static{

   client = new HbaseClientCopy();
   }
   ...
   try{
      ...
      sql = "select v_content_key, zone_id, v_zfilename_digest, zone_file, update_time, status, n_src, __version from " + tblname + " limit 10";

      System.out.println("mysql query string: " +  sql);

      ResultSet rs = stmt.executeQuery(sql);

      System.out.println("no exception during JDBC query start to write to hbase ");
